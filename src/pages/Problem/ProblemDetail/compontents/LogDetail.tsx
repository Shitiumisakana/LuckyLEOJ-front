...
import {getQuestionSubmitVoByIdUsingGET} from "@/services/lucky-oj-backend/question/questionController";
...
const LogDetail: React.FC<LogDetailProps> = ({targetSubmitId, logHeight, afterClose})=>{
  ...
  useEffect(() => {
    if(targetSubmitId > 0){
      getQuestionSubmitVoByIdUsingGET({
        id: targetSubmitId
      }).then(res => {
        if(res.data){
          setLoading(false);
  ...
  const isAccepted = problemSubmit?.judgeInfo?.message === 'Accepted';

  const finalStatusText = (problemSubmit?.status === 2 && isAccepted) 
  ? "通过" 
  : submitStatusText.get(problemSubmit?.status || 0);

  const finalStatusColor = (problemSubmit?.status === 2 && isAccepted) 
  ? Color.EASY 
  : submitStatusColor.get(problemSubmit?.status || 2);
            ...
            <div style={{display: 'flex'}}>
              ...
              <div style={{float: 'right'}}>
                ...
                <Button
                  size='large'
                  type="text"
                  style={{color: finalStatusColor, fontSize: 18, padding: '0 16px'}}
                  icon={isAccepted ? submitStatusIcon.get(3) : submitStatusIcon.get(problemSubmit?.status || 2)}
>
  {finalStatusText}
                </Button>
              </div>
            ...
            {
              problemSubmit.status === SUBMIT_STATUS.FAILED && !isAccepted &&
